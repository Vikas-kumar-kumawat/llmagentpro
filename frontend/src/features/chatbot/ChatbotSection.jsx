      ...
      <div className="flex-1 max-w-4xl mx-auto w-full flex flex-col overflow-y-auto py-1">
        {messages.length === 0 ? (
          <div className="my-auto space-y-3 sm:space-y-6 animate-fadeIn w-full">
            <div className="text-left border-b border-[#27272a] pb-2 sm:pb-4">
              <h2 className="text-xs sm:text-lg font-semibold text-white tracking-tight font-['Plus_Jakarta_Sans',sans-serif]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3.5">
              <div 
                ...
                className="p-2.5 sm:p-4 rounded-lg sm:rounded-xl border border-[#27272a] bg-[#09090b] hover:border-white hover:bg-[#18181b] transition-all duration-200 cursor-pointer group flex flex-col justify-between space-y-1.5 sm:space-y-3"
              >
                <div className="flex items-center justify-between">
                  <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg flex items-center justify-center border bg-[#000000] text-white border-[#27272a] group-hover:border-white transition-colors">
                    <MessageSquare className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                  </div>
                  <span className="text-[9px] sm:text-xs font-medium text-[#a1a1aa] group-hover:text-white flex items-center gap-1 transition-colors">
                    <span>LAUNCH</span> <ArrowRight className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-white" />
                  </span>
                </div>
                <div>
                  <h3 className="text-[11px] sm:text-sm font-semibold text-white group-hover:text-white transition">Feedback Collector</h3>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <h3 className="text-[11px] sm:text-sm font-semibold text-white group-hover:text-white transition">Recharge Reminder</h3>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <h3 className="text-[11px] sm:text-sm font-semibold text-white group-hover:text-white transition">New Offers Call</h3>
                </div>
              </div>
              ...
              >
                ...
                <div>
                  <h3 className="text-[11px] sm:text-sm font-semibold text-white group-hover:text-white transition">Competitor Price Monitor</h3>
                </div>
              </div>
            </div>
          </div>
        ) : (
          <div className="w-full space-y-2.5 sm:space-y-4 px-1 my-auto max-w-4xl mx-auto">
            {messages.map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div
                  className={`max-w-[88%] sm:max-w-[80%] rounded-xl p-2.5 sm:p-4 space-y-1 sm:space-y-1.5 shadow-sm border ${
                    msg.sender === 'human'
                ...
                >
                  <div className="flex items-center justify-between gap-3 pb-1 border-b border-zinc-800/40">
                    <span className={`font-semibold flex items-center gap-1.5 text-[9px] sm:text-xs uppercase tracking-wider ${msg.sender === 'human' ? 'text-black' : 'text-white'}`}>
                      {msg.sender === 'agent' ? <><Bot className="w-3.5 h-3.5 text-white" /> BFibernet AI</> : 'You'}
                    </span>
                    <span className={`text-[9px] sm:text-xs ${msg.sender === 'human' ? 'text-zinc-600' : 'text-[#a1a1aa]'}`}>{msg.time}</span>
                  </div>
                  <p className="text-[11px] sm:text-sm font-medium">{msg.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <form onSubmit={handleSend}>
          <div className="p-2 sm:p-3 rounded-xl border border-[#27272a] bg-[#09090b] focus-within:border-white transition flex items-center gap-2 sm:gap-3">
            ...
            <button
              type="submit"
              disabled={!input.trim()}
              className={`px-3 sm:px-6 py-2 sm:py-3 rounded-lg text-xs sm:text-sm font-semibold uppercase tracking-wider transition flex items-center justify-center gap-1.5 shrink-0 cursor-pointer border ${
                input.trim()
