...
def generate_ai_response(customer_text: str, customer: Optional[Dict[str, Any]] = None) -> str:
    ...
    if not ai_text:
        ...
        bye_words = ["bye", "goodbye", "thank you", "thanks", "that's all", "done", "no", "that is all", "धन्यवाद", "कोनी"]
...
def twilio_feedback():
    """Processes customer speech result / keypress digits and renders conversational response."""
    customer_id = request.args.get("customer_id")
    called_phone = request.form.get("To") or request.form.get("From")
    speech_result = request.form.get("SpeechResult", "").strip()
    digits = request.form.get("Digits", "").strip()
    customer_text = speech_result or (f"Pressed key {digits}" if digits else "")
    ...
    if customer:
        ...
        customer.setdefault("transcript", []).append({
            "speaker": "customer", 
            "text": customer_text,
