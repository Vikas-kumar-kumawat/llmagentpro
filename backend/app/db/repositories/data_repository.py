...
class DataRepository:
    ...
    def save_feedback(customer_name: str, phone: str, rating: int, feedback_text: str, sentiment: str, category: str, followup_needed: bool):
        ...
        # Automatically store contact info in contacts table if not already present
        if phone:
            cursor.execute("SELECT id FROM contacts WHERE phone = ?", (phone,))
            if not cursor.fetchone():
                cursor.execute("INSERT INTO contacts (name, phone, created_at) VALUES (?, ?, ?)", (customer_name, phone, now_str))
            else:
                cursor.execute("UPDATE contacts SET name = ? WHERE phone = ?", (customer_name, phone))

        # Check if customer entry already exists for this phone to avoid duplicate rows
        cursor.execute("SELECT id, transcript, feedback_text FROM feedback_entries WHERE phone = ? ORDER BY id DESC LIMIT 1", (phone,))
        existing_row = cursor.fetchone()

        if existing_row:
            feedback_id = existing_row["id"]
            existing_transcript = []
            if existing_row["transcript"]:
                try:
                    existing_transcript = json.loads(existing_row["transcript"])
                except Exception:
                    pass

            old_text = existing_row["feedback_text"]
            if old_text and old_text != feedback_text and old_text not in ("No previous feedback recorded.", "Calling customer for feedback..."):
                existing_transcript.append({
                    "speaker": "history",
                    "name": "Previous Feedback History",
                    "time": now_str,
                    "text": old_text
                })
            ...
            cursor.execute("""
                ...
                SET customer_name = ?, rating = ?, feedback_text = ?, sentiment = ?, category = ?, followup_needed = ?, transcript = ?, created_at = ?
                WHERE id = ?
            """, (customer_name, rating, feedback_text, sentiment, category, 1 if followup_needed else 0, json.dumps(existing_transcript) if existing_transcript else existing_row["transcript"], now_str, feedback_id))
